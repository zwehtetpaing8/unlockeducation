...
function offsetArrow(x1, y1, x2, y2, offset, colorClass, text, textX, textY, textColorClass) {
  const dx = x2 - x1;
  const dy = y2 - y1;
  ...
  const px1 = nx2 - headLen * Math.cos(angle - headAngle);
  const py1 = ny2 - headLen * Math.sin(angle - headAngle);
  const px2 = nx2 - headLen * Math.cos(angle + headAngle);
  const py2 = ny2 - headLen * Math.sin(angle + headAngle);

  // We slightly shorten the line so it doesn't poke out of the polygon
  const lineX2 = nx2 - 2 * Math.cos(angle);
  const lineY2 = ny2 - 2 * Math.sin(angle);

  const fillClass = colorClass.includes('stroke-slate-900') 
    ? 'fill-slate-900 dark:fill-slate-100' 
    : colorClass.replace('stroke-', 'fill-');

  return `          <line x1="${nx1.toFixed(1)}" y1="${ny1.toFixed(1)}" x2="${lineX2.toFixed(1)}" y2="${lineY2.toFixed(1)}" className="${colorClass}" strokeWidth="2" />
          <polygon points="${nx2.toFixed(1)},${ny2.toFixed(1)} ${px1.toFixed(1)},${py1.toFixed(1)} ${px2.toFixed(1)},${py2.toFixed(1)}" className="${fillClass} stroke-none" />
...
${offsetArrow(60, 85, 140, 65, 8, 'stroke-[#ca8a04]', '$\\\\overrightarrow{AB}$', 90, 45, 'text-[#ca8a04]')}

${offsetArrow(60, 85, 260, 35, -8, 'stroke-slate-900 dark:stroke-slate-100', '$\\\\overrightarrow{AC}$', 160, 45, 'text-slate-900 dark:text-slate-100')}
