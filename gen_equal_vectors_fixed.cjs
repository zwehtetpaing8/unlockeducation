...
// Draw a grid
let gridHTML = `<g className="stroke-slate-200 dark:stroke-slate-800" strokeWidth="1">`;
for(let i=0; i<=300; i+=20) {
    gridHTML += `\n  <line x1="${i}" y1="0" x2="${i}" y2="120" />`;
}
for(let i=0; i<=120; i+=20) {
    gridHTML += `\n  <line x1="0" y1="${i}" x2="300" y2="${i}" />`;
}
gridHTML += `\n</g>`;

// Equal vectors
const v1_start = { x: 40, y: 100 };
const v1_end = { x: 140, y: 40 };

const v2_start = { x: 140, y: 80 };
const v2_end = { x: 240, y: 20 };

console.log(gridHTML);

console.log(arrowLine(v1_start.x, v1_start.y, v1_end.x, v1_end.y, 'yellow', 12));
console.log(labelWithArrow((v1_start.x + v1_end.x)/2 - 15, (v1_start.y + v1_end.y)/2 - 10, 'a', 'yellow', 10));

console.log(arrowLine(v2_start.x, v2_start.y, v2_end.x, v2_end.y, 'blue', 12));
console.log(labelWithArrow((v2_start.x + v2_end.x)/2 + 15, (v2_start.y + v2_end.y)/2 + 25, 'b', 'blue', 10));

const dot = (x, y) => `<circle cx="${x}" cy="${y}" r="3" className="fill-slate-800 dark:fill-slate-200 stroke-none" />`;
console.log(dot(v1_start.x, v1_start.y));
console.log(dot(v2_start.x, v2_start.y));
