...
const startStr = 'သွေးအမျိုးအစားက $\\text{A, B, AB, O}$ ဆိုပြီး $4$ မျိုးရှိသည်။';
const endStr = 'Note: $0$ and $100$ do not contain the digit $5$ exactly once, so we only consider $1$ to $99$.)';

const startIdx = content.indexOf(startStr);
const endIdx = content.indexOf(endStr);

if (startIdx !== -1 && endIdx !== -1) {
  const targetToReplace = content.substring(startIdx, endIdx + endStr.length);
...
(Note: $0$ and $100$ do not contain the digit $5$ exactly once, so we only consider $1$ to $99$.)`;
  ...
  content = content.replace(targetToReplace, restoredText);
  fs.writeFileSync('src/data/chapter5_content.ts', content);
  console.log("Restored missing Examples 2, 3, 4, 5 using substring!");
} else {
